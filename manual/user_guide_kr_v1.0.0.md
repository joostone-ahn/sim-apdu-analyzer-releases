# SIM-APDU-Analyzer 사용자 가이드 v1.0.0
...
- 같은 파일의 같은 위치(REC#, OFS)에 서로 다른 값이 기록된 경우 감지됩니다
- 변경 전(READ) 값과 변경 후(UPDATE) 값이 각각 별도 행으로 표시됩니다
- 해당 파일/레코드의 모든 행이 노란색으로 표시됩니다
...
- **Decode** (기본): pySim 기반 JSON 디코딩 (160+ EF 지원). 특수 뷰로 PLMN 테이블, Service 테이블 (UST/IST/EST), ACC, ARR, URSP 규칙 제공. 디코더가 없는 EF는 "No decoded data" 표시
- **Raw**: 원본 hex 문자열 표시
