# SIM-APDU-Analyzer User Guide v1.0.0
...
- Detected when the same file/record position (REC#, OFS) has different values
- Both the original (READ) and updated (UPDATE) values shown as separate rows
- All rows for that file/record are highlighted
...
- **Decode** (default): pySim-based JSON decoding (160+ EFs). Includes special views for PLMN tables, Service tables (UST/IST/EST), ACC, ARR, URSP rules. Shows "No decoded data" for unsupported EFs
